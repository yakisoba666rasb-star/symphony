defmodule SymphonyElixir.TestSupport do
  ...
  defp workflow_content(overrides) do
    config =
      Keyword.merge(
        [
          ...
          codex_allow_linear_graphql_mutations: false,
    ...
    tracker_all_projects = Keyword.get(config, :tracker_all_projects)
    ...
    codex_allow_linear_graphql_mutations = Keyword.get(config, :codex_allow_linear_graphql_mutations)
    ...
    sections =
      [
        ...
        "  all_projects: #{yaml_value(tracker_all_projects)}",
  ...
  defp stall_yaml(true, 900_000, 900_000), do: nil

  defp stall_yaml(enabled, threshold_ms, review_threshold_ms) do
    [
      "stall:",
      "  enabled: #{yaml_value(enabled)}",
      "  threshold_ms: #{yaml_value(threshold_ms)}",
      "  review_threshold_ms: #{yaml_value(review_threshold_ms)}"
    ]
    |> Enum.join("\n")
  end
